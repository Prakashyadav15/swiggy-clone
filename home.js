...
function Home(){
    ...
    //  const allsimilaritems=ItemsList.flatMap(item=>
    //    ( item.similarItems.map(similar=>({...similar,parentId:item.id})))
    //  )
    ...
    //         const filtered=allsimilaritems.filter(item=>
    //             item.name.toLowerCase().includes(lowersearch)
    //         )
    //         setfilteredsearch(filtered)
    //     }
    // },[search,allsimilaritems]);
